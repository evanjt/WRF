...
bool load_station_state(const std::string& station_key, SnowStation* station,
                        const mio::Coords& position,
                        const std::string& stationID,
                        const std::string& stationName, SnowpackIO* io,
                        std::mutex& io_mutex, int wrf_domain_id, int i_grid,
                        int j_grid) {
  if (!io) {
    printf(
  ...
  try {
    // CRYOWRF C++ naming pattern: snpack_{grid_id}_{I}_{J}.sno (from
    // Coupler.cpp line 613) Let SNOWPACK handle the path through SNOWPATH
    // configuration
    ...
    station->initialize(ssdata,
                        0);  // Initialize SnowStation from SN_SNOWSOIL_DATA

    // Ensure proper initialization of layer properties
    for (size_t e = 0; e < station->getNumberOfElements(); e++) {
      // Ensure k and c vectors have proper size (3 elements: TEMPERATURE,
      // SEEPAGE, SETTLEMENT)
      ...
      for (size_t i = 0; i < 3; i++) {
        if (std::isnan(station->Edata[e].k[i]) ||
            station->Edata[e].k[i] != station->Edata[e].k[i]) {
          station->Edata[e].k[i] = 0.0;
        }
        if (std::isnan(station->Edata[e].c[i]) ||
            station->Edata[e].c[i] != station->Edata[e].c[i]) {
...
void initialize_new_station(SnowStation* station, const mio::Coords& position,
                            const std::string& stationID,
                            const std::string& stationName, double height,
...
SnowpackBridgeObjects create_station_objects(const MeteoInput& input,
  ...
  objects.station = bridge.get_or_create_snowstation(
      input.i_grid, input.j_grid, 1, input.wrf_lat, input.wrf_lon,
      input.height);
  ...
  objects.instance = bridge.get_or_create_snowpack_instance(
      input.i_grid, input.j_grid, input.wrf_lat, input.wrf_lon, input.height);
...
void ensure_station_initialized(SnowStation* station, const MeteoInput& input,
                                SnowpackBridge& bridge) {
  if (station->getNumberOfElements() == 0) {
    ...
    std::string stationID = constants.station_id_prefix + "_" +
                            std::to_string(input.i_grid) + "_" +
                            std::to_string(input.j_grid);
    std::string stationName = "WRF Grid Point " + std::to_string(input.i_grid) +
                              "," + std::to_string(input.j_grid);
    ...
    if (bridge.start_from_file_enabled()) {
      loaded_from_file = load_station_state(
          generate_grid_key(input.i_grid, input.j_grid), station, position,
          stationID, stationName, bridge.get_io(), bridge.io_mutex(), 1,
          input.i_grid, input.j_grid);
    }

    if (!loaded_from_file) {
      // Initialize fresh station
      initialize_new_station(station, position, stationID, stationName,
                             input.height, input.i_grid, input.j_grid);
    }
  }
}

bool save_station_state(SnowStation* station, const mio::Date& current_time,
                        SnowpackIO* io, std::mutex& io_mutex, int i_grid,
                        int j_grid) {
  if (!station || !io) {
  ...
  try {
    ...
    printf(
        "SNOWPACK-INFO: Saved .sno state for grid (%d,%d) using SnowpackIO\n",
        i_grid, j_grid);
    return true;
...
void save_all_station_states(
    const std::map<std::string, std::unique_ptr<SnowStation>>& stations,
    const mio::Date& current_time, SnowpackIO* io, std::mutex& io_mutex) {
  if (!io) {
