...
// Global configuration cache to ensure all SnowpackConfig objects share the
// same data
namespace {
...
mio::Config SnowpackConfigManager::loadConfiguration(
  ...
  try {
    ...
    printf(
        "SNOWPACK-DEBUG: Loading and caching new SnowpackConfig from %s "
        "(absolute: %s)\n",
    ...
    g_cached_config =
        std::make_shared<mio::Config>(config);  // Cache the base mio::Config
    g_last_config_path = resolved_path;

    const double calculation_step_length =
        config.get("CALCULATION_STEP_LENGTH", "Snowpack");
    ...
    printf(
        "SNOWPACK-CONFIG: METEO_STEP_LENGTH set to %s seconds (from "
    ...
    printf(
        "SNOWPACK-ERROR [C++/SnowpackConfigManager]: Failed to load %s: %s\n",
...
void SnowpackConfigManager::validateConfiguration(const mio::Config& cfg) {
  // Check for essential SNOWPACK parameters that will be read by SNOWPACK
  // components
  ...
  for (const auto& param : required_params) {
    try {
      ...
      printf(
          "SNOWPACK-ERROR [C++/SnowpackConfigManager]: Missing required "
          "parameter %s::%s\n",
          param.second.c_str(), param.first.c_str());
      throw std::runtime_error("Configuration validation failed: missing " +
                               param.first);
    }
  }
}
