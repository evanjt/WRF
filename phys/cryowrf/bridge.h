...
class SnowpackBridge {
 ...
 public:
  ...
  bool is_config_initialized() const {
    return config_initialized_.load(std::memory_order_acquire);
  }
  ...
  bool is_time_initialized() const {
    return time_initialized_.load(std::memory_order_acquire);
  }
  const mio::Date& get_current_time() const { return current_simulation_date_; }

  // Object management
  SnowStation* get_or_create_snowstation(int i_grid, int j_grid,
                                         int wrf_domain_id, double wrf_lat,
                                         double wrf_lon, double wrf_alt);
  Snowpack* get_or_create_snowpack_instance(int i_grid, int j_grid,
                                            double wrf_lat, double wrf_lon,
                                            double wrf_alt);
  ...
  void execute_snowpack(const MeteoInput& input, SnowpackOutput& output,
