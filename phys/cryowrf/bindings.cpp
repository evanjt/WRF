...
void initialize_wrf_simulation_time_c(int start_year, int start_month,
  ...
  try {
    ...
    bridge.initialize_time(start_year, start_month, start_day, start_hour,
                           start_minute);

  } catch (const std::exception& e) {
    printf(
        "SNOWPACK-FATAL [C++/snowpack_wrf_bridge.cpp]: Failed to initialize "
        "time: %s\n",
        e.what());
    std::abort();
  }
...
void snowpack_physics_c(const MeteoInput* input, SnowpackOutput* output,
...
void extract_snowpack_layers_c(int i_grid, int j_grid, float* layer_temps,
                               float* layer_thick, float* layer_voli,
                               float* layer_volw, float* layer_volv,
                               float* layer_rg, float* layer_rb,
                               float* layer_dd, float* layer_sp,
                               float* layer_cdot, float* layer_meta,
                               float* layer_depd, float* layer_graintype,
                               float* layer_mk, int* n_layers) {
...
void extract_snowpack_layers_c_(int i_grid, int j_grid, float* layer_temps,
                                float* layer_thick, float* layer_voli,
                                float* layer_volw, float* layer_volv,
                                float* layer_rg, float* layer_rb,
                                float* layer_dd, float* layer_sp,
                                ...
                                float* layer_depd, float* layer_graintype,
                                float* layer_mk, int* n_layers) {
  extract_snowpack_layers_c(
      i_grid, j_grid, layer_temps, layer_thick, layer_voli, layer_volw,
      layer_volv, layer_rg, layer_rb, layer_dd, layer_sp, layer_cdot,
      layer_meta, layer_depd, layer_graintype, layer_mk, n_layers);
}
